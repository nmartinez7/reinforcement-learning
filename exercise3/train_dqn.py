...
LUNARLANDER_CONFIG = {
    "eval_freq": 2000, # HOW OFTEN WE EVALUATE (AND RENDER IF RENDER=TRUE)
    "eval_episodes": 30,  # DECREASING THIS MIGHT REDUCE EVALUATION ACCURACY; BUT MAKES IT EASIER TO SEE HOW THE POLICY EVOLVES OVER TIME (BY ENABLING RENDER ABOVE)
    "learning_rate": 5e-4,
    "hidden_size": (64, 64),
...
def train(env: gym.Env, config, output: bool = True) -> Tuple[List[float], List[float]]:
    ...
    if config["plot_loss"]:
        ...
        plt.ylabel("DQN Loss", fontsize=15)
