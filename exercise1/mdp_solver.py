    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        #print("Policy: {}".format(self.mdp.P))
        
        #print("Rewards: {}".format(self.mdp.R))
        
        #print("States: {}".format(self.mdp.states))
        
        #print("Actions: {}".format(self.mdp.actions))
        ...
        while True:
            ...
            delta = 0.
            #print("iteration: {}, values: {}".format(iteration, V))
            ...
            for state in range(self.state_dim):
                ...
                for action in range(self.action_dim):
                    for end_state in range(self.state_dim):
                        #V[state] += self.mdp.P[state,action,end_state]*(self.mdp.R[state,action,end_state]
                        #+ self.gamma*V[end_state])
                        action_values[action] += self.mdp.P[state,action,end_state]*(self.mdp.R[state,action,end_state]
                        + self.gamma*V[end_state])
            ...
            if delta < theta:
                print("Done, delta is {:g}, at iteration #: {}".format(delta, iteration))
                break
        ...
        for state in range(self.state_dim):            
            for action in range(self.action_dim):
                for end_state in range(self.state_dim):
                    ...
                    state_action_values[state,action] += self.mdp.P[state,action,end_state]*(self.mdp.R[state,action,end_state]
                    + self.gamma*V[end_state])                      
            
            
            greedy_action = np.argmax(state_action_values[state])
            
            for action in range(self.action_dim):
                policy[state,action] = 1 if action == greedy_action else 0
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for state in range(self.state_dim):
                ...
                for action in range(self.action_dim):
                    for end_state in range(self.state_dim):
                        V[state] += policy[state, action]*self.mdp.P[state,action,end_state]*(self.mdp.R[state,action,end_state]
                        + self.gamma*V[end_state])
                        #V[state] += policy[state, action]*self.mdp.P[state,action,end_state]*(self.mdp.R[state,action,end_state]
                        #+ self.gamma*V[end_state])
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for state in range(self.state_dim):
            ...
            randrow = np.random.random(self.action_dim)
            policy[state,:] = randrow/sum(randrow)
            
        #print("ppolicy:{}".format(policy))
            
        #policy[:,0] = 1
        
        old_action_bool = [False for j in range(self.state_dim)]
            ...
            for state in range(self.state_dim):
                ...
                for action in range(self.action_dim):
                    for end_state in range(self.state_dim):
                        ...
                        #state_action_values[state,action] += policy[state,action]*self.mdp.P[state,action,end_state]*(self.mdp.R[state,action,end_state] +  
                        #          self.gamma*V[end_state])   
                        
                        
                # Pick the action with the highest returns:
                greedy_action = np.argmax(state_action_values[state,:])
                ...
                if old_action == greedy_action:
                    old_action_bool[state] = True
                    
            #print(old_action_bool)
            # Break if ALL states stop changing:
            if sum(old_action_bool) == self.state_dim:
